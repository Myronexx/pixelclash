...
function Admintools() {
  ...
  // VIP list fetch
  const data = new FormData();
  data.append('getviplist', '1');

  fetch(api`/api/modtools`, {
    method: 'POST',
    credentials: 'include',
    body: data,
  })
            ...
            callback={(name) => {
              ...
              fetch(api`/api/modtools`, {
                ...
                .then(async (resp) => {
                  ...
                  if (resp.ok) {
                    setVipList(vipList.filter((v) => v !== name));
                  }
                })
          ...
          onClick={() => {
            ...
            fetch(api`/api/modtools`, {
              ...
              .then(async (resp) => {
                ...
                if (resp.ok) {
                  setVipList([...vipList, vipName]);
                }
              })
              .finally(() => setSubmitting(false));
          }}
        >
          {(submitting) ? '...' : 'Submit VIP'}
        </button>

        <br /><br />

      </div>  {/* 🔥 VIP BÖLÜMÜ BU DİV İÇİNDE OLMALIYDI */}
    </div>
  );
}
