...
import { User, USERLVL } from '../../data/sql/index.js';
...
router.post('/', async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.getviplist) {
      const users = await User.findAll({ where: { lvl: USERLVL.VIP } });
    ...
    if (req.body.getviplist) {
      const users = await User.findAll({ where: { lvl: USERLVL.VIP } });
      const list = users.map((u) => [u.id, u.name]);
      res.status(200).json(list);
      return;
    }
    // --- VIP KALDIRMA ---
    if (req.body.remvip) {
      const name = req.body.remvip.trim();
    ...
    if (!user) {
      ...
      user.lvl = USERLVL.USER;
      await user.save();

      res.status(200).send(`VIP removed from ${name}`);
      return;
    }

      if (req.body.makevip) {
    const raw = (req.body.makevip || '').trim();

    // Hem ID hem isimle çalışsın
    let where = {};
    if (/^\d+$/.test(raw)) {
      // Sadece rakamsa ID olarak dene
      where = { id: parseInt(raw, 10) };
    } else {
      // Değilse isim olarak dene
      where = { name: raw };
    }
    ...
    if (!user) {
      // 400 yerine 200 dönelim ki hem popup düzgün görünsün
      res.status(200).send(`User not found: ${raw}`);
      return;
    }

    user.lvl = USERLVL.VIP;
    await user.save();

    res.status(200).send(`VIP given to ${user.name} (${user.id})`);
    return;
  }
